...
setwd(find_root("lipidvisualization.Rproj"))
setwd("analyses/data")

GDM_Negative <- read.csv("GDM vs. Ctrl_Negative.csv", stringsAsFactors = F)
GDM_Positive <- read.csv("GDM vs. Ctrl_Positive.csv", stringsAsFactors = F)

All_lipids <- rbind(GDM_Positive, GDM_Negative)
...
df <- read.csv("All_Lipid_SMILES.csv", stringsAsFactors = F, header = T)
...
# Dendrogram of only significantly differntiable disease vs. control lipids

df <- read.csv("Lipid_SIG_SMILES.csv", stringsAsFactors = F, header = T)
